...
@cliCommandItem()
export class JsonFactoryCommand
  extends CliCommand<CommandName, CommandArgvName>
  implements OnImplement
{
  cwd: string;
  ...
  get argvs(): CommandArgvItem<CommandArgvName>[] {
    return [
      ...
      {
        name: "no-module",
        description: "not used module directory",
        alias: "nomod",
        type: "boolean",
      },
    ];
  ...
  async init() {
    ...
    if (this.hasArgv("no-module")) {
      this.sourceAppStrongFBPath = path.join(this.source);
    }
  ...
  async implement(): Promise<boolean> {
    ...
    // =>create 'info.ts' file
    fs.writeFileSync(
      path.join(this.sourceAppStrongFBPath, "info.ts"),
      `
  ...
  async copyTemplates() {
    // =>copy common files
    fs.mkdirSync(path.join(this.sourceAppStrongFBPath, "common"), {
      recursive: true,
    });
    let files = fs.readdirSync(path.join(this.templatesPath, "common"), {
      withFileTypes: true,
    });
    for (const f of files) {
      if (f.isFile()) {
        fs.copyFileSync(
          path.join(this.templatesPath, "common", f.name),
          path.join(this.sourceAppStrongFBPath, "common", f.name)
        );
      }
    }
    ...
    if (fs.existsSync(path.join(this.templatesPath, "common", "json"))) {
      let files = fs.readdirSync(
        path.join(this.templatesPath, "common", "json"),
        { withFileTypes: true }
      );
      for (const f of files) {
        if (f.isFile()) {
          fs.copyFileSync(
            path.join(this.templatesPath, "common", "json", f.name),
            path.join(this.sourceAppStrongFBPath, "common", f.name)
          );
        }
      }
    ...
    if (fs.existsSync(path.join(this.templatesPath, "json"))) {
      let dirs = fs.readdirSync(path.join(this.templatesPath, "json"), {
        withFileTypes: true,
      });
      for (const dir of dirs) {
        if (dir.isFile()) {
          fs.copyFileSync(
            path.join(this.templatesPath, "json", dir.name),
            path.join(this.sourceAppStrongFBPath, dir.name)
          );
        }
      }
    }
    // =>compile & copy common helper files
    let helperFiles = fs.readdirSync(
      path.join(this.templatesPath, "common", "helpers"),
      { withFileTypes: true }
    );
    fs.mkdirSync(path.join(this.sourceAppStrongFBPath, "common", "helpers"), {
      recursive: true,
    });
    for (const f of helperFiles) {
      if (!f.isFile()) continue;
      await TEM.saveRenderFile(
        path.join(this.templatesPath, "common", "helpers", f.name),
        path.join(this.sourceAppStrongFBPath, "common", "helpers"),
        {
          data: {
  ...
  async copyWidgetsTemplates() {
    ...
    for (const widDir of widgetsDirs) {
      ...
      if (
        widDir.isDirectory() &&
        fs.existsSync(path.join(widgetDirPath, "json"))
      ) {
        // =>create widget dir folder
        fs.mkdirSync(appWidgetDirPath, { recursive: true });
        let widgetFiles = fs.readdirSync(widgetDirPath, {
          withFileTypes: true,
        });
        // =>add loaded widget
        ...
        for (const widFile of widgetFiles) {
          if (
            widFile.name.indexOf("component.ts") > -1 ||
            widFile.name.indexOf("component.html") > -1 ||
            widFile.name.indexOf("component.scss") > -1
          )
            continue;
          // =>if common files
          if (widFile.isFile()) {
            fs.copyFileSync(
              path.join(widgetDirPath, widFile.name),
              path.join(appWidgetDirPath, widFile.name)
            );
          }
        }
        // =>overwrite json files
        let widgetJsonFiles = fs.readdirSync(path.join(widgetDirPath, "json"), {
          withFileTypes: true,
        });
        for (const widFile of widgetJsonFiles) {
          ...
          if (widFile.isFile()) {
            fs.copyFileSync(
              path.join(widgetDirPath, "json", widFile.name),
              path.join(appWidgetDirPath, widFile.name)
            );
          }
        }
