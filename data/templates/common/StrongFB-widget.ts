...
import {
  StrongFBLayoutBuilderSchema,
  StrongFBLayoutBuilderWidgetFunction,
} from "./StrongFB-layout-builder-types";
...
import {
  APIRequest,
  FormFieldMetaData,
  StrongFBJsonApiRequestSchema,
} from "./StrongFB-interfaces";
  ...
  implements AfterViewInit
{
  ...
  public valueChanges$ = new BehaviorSubject<[boolean, any]>([
    false,
    undefined,
  ]);
  ...
  private _generateWidgetId() {
    ...
    this._widgetId =
      `strong_fb_${this.prefixId}_widget_` +
      new Date().getTime() +
      "_" +
      ...
      .subscribe(async () => {
        ...
        try {
          if (
            this.widgetForm &&
            this.widgetHeader &&
            this.widgetHeader["_showCallback"]
          ) {
            let beforeStateShow = this.show;
            this.show = await this.widgetHeader["_showCallback"].call(
              this.widgetForm,
              this.widgetHeader
            );
            if (beforeStateShow !== this.show) {
              // =>set show state
              this.widgetForm.setFormFieldMeta(
                this.widgetHeader["_formFieldName"],
                { is_show: this.show }
              );
  ...
  listenOnFormFieldChange(valueField: keyof SCHEMA) {
    ...
    if (this.widgetForm) {
      ...
      this.widgetForm["_formFieldValuesUpdated$"]
        .pipe(takeUntil(this.destroy$))
        .subscribe((it) => {
          if (!it) return;
          ...
          if (
            this.widgetHeader["_formFieldName"] &&
            this.widgetForm["_formFieldValues"][
              this.widgetHeader["_formFieldName"]
            ] !== undefined
          ) {
            this.schema[valueField] =
              this.widgetForm["_formFieldValues"][
                this.widgetHeader["_formFieldName"]
              ];
          }
  ...
  makeDirtyField() {
    if (this.widgetForm && this.widgetHeader["_formFieldName"]) {
      // => if before not dirty
      let beforeDirty = this.widgetForm.formFieldMeta(
        this.widgetHeader["_formFieldName"]
      )?.is_dirty;
  ...
  async loadDynamicWidgets(
    ...
    widgetsSchema: {
      ...
      widgetHeaders?: StrongFBLayoutBuilderWidgetFunction<
        StrongFBBaseWidgetHeader[]
      >[];
    },
    ...
    if (widgetsSchema.widgets) {
      for (const widgetFunction of widgetsSchema.widgets) {
        let widget = (await widgetFunction.call(
          form
        )) as StrongFBBaseWidgetHeader;
        if (!widget) {
          SFB_warn(
            `can not find widget instance from '${widgetFunction.name}' function`
          );
          continue;
        }
    ...
    if (widgetsSchema.widgetHeaders) {
      for (const widgetsFunction of widgetsSchema.widgetHeaders) {
        let widgets = (await widgetsFunction.call(
          form
        )) as StrongFBBaseWidgetHeader[];
        for (const widget of widgets) {
          // =>if has function name and no name property
          if (
            form[widgetsFunction.name] &&
            !widget["_name"] &&
            widget["name"]
          ) {
    ...
    for (const widget of widgetHeaders) {
      ...
      let widgetElement: HTMLElement = <HTMLElement>(
  ...
  /******************************************* */
  async sendRequestByJsonApi(
    http: StrongFBHttpService,
    jsonApiSchema: StrongFBJsonApiRequestSchema
  ) {
    let data: APIRequest = {
