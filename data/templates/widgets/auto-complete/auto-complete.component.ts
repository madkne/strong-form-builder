...
export class StrongFBAutoCompleteWidgetComponent extends StrongFBBaseWidget<AutoCompleteSchema> {
  ...
  @ViewChild(Object, { read: ElementRef })
  ...
  override async onInit() {
    ...
    this.widgetHeader["_updateOptions$"]
    ...
    this.valueChanges$
      .pipe(takeUntil(this.destroy$))
      .subscribe(async (it: [boolean, any]) => {
        if (!it || !it[0]) return;
        // =>if multiple values
        if (this.schema.multiple && Array.isArray(this.schema.value)) {
          for (const val of this.schema.value) {
            ...
            if (this.schema.loadTextByValue) {
              text = await this.schema.loadTextByValue(
                val,
                this.widgetHeader as any
              );
            }
  ...
  async loadOptions() {
    ...
    if (this.schema.loadOptions) {
      this.options = await this.schema.loadOptions.call(
        this.widgetForm,
        this.widgetHeader,
  ...
  changeEvent(event: string | string[]) {
    ...
    this.emitSchemaChangeEvent();
  }

  emitSchemaChangeEvent() {
    if (!this.schema.change) return;
    this.schema.change.call(
      this.widgetForm,
      this.schema.value,
      this.widgetHeader
    );
  ...
  onRemove(event) {
    ...
    if (option) {
      this.schema.selectedOptions.splice(
        this.schema.selectedOptions.findIndex((i) => i.text === event.text),
        1
      );
    }
    ...
    if (
      ...
      (this.schema.value as string[]).indexOf(removedValue) > -1
    ) {
      (this.schema.value as string[]).splice(
        (this.schema.value as string[]).indexOf(removedValue),
        1
      );
  ...
  toggleSelectOption(option: AutoCompleteOption) {
    ...
    if (this.isSelected(option.value)) {
      ...
      if (this.schema.multiple) {
        (this.schema.value as string[]).splice(
          (this.schema.value as string[]).indexOf(option.value),
          1
        );
      } else {
        this.schema.value = undefined;
      ...
      this.schema.selectedOptions.splice(
        this.schema.selectedOptions.findIndex((i) => i.value === option.value),
        1
      );
    } else {
